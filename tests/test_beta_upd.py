...
def _make_test_inputs(seed=42, nC=5, nG=50, nK=3):
    ...
    beta_init = np.random.randn(nC, nG, nK).astype(np.float32) * 0.1
    ...
    return beta_init, mu, Ac, eta, theta, gamma, precision, counts, class_prob
...
def test_beta_upd_mathematical_consistency():
    ...
    """
    beta_init, mu, Ac, eta, theta, gamma, precision, counts, class_prob = _make_test_inputs()

    # Run Naive
    beta_naive = beta_upd_naive(beta_init, mu, Ac, eta, theta, gamma, precision, counts)

    # Run Optimized; high max_iter so we hit the float32 floor against naive
    beta_opt = beta_upd_optimized(beta_init.copy(), mu, Ac, eta, theta, gamma,
                            precision, counts, class_prob, max_iter=50)

    max_diff = np.abs(beta_naive - beta_opt).max()
...
def test_beta_upd_gpu_consistency():
    ...
    beta_gpu = beta_upd_gpu(beta_init.copy(), mu, Ac, eta, theta, gamma,
                      precision, counts, class_prob, max_iter=50)

    max_diff = np.abs(beta_naive - beta_gpu).max()
...
def test_beta_upd_gpu_matches_cpu_optimized():
    """
    Verifies that GPU PCG produces the same beta as the CPU optimized PCG
    ...
    beta_cpu = beta_upd_optimized(beta_init.copy(), mu, Ac, eta, theta, gamma,
                            precision, counts, class_prob, max_iter=20)
    beta_gpu = beta_upd_gpu(beta_init.copy(), mu, Ac, eta, theta, gamma,
                      precision, counts, class_prob, max_iter=20)

    max_diff = np.abs(beta_cpu - beta_gpu).max()
...
def _make_realistic_inputs(seed=42, nC=50, nG=50, nK=3, mu_max=65.0,
    ...
    beta_init = np.zeros((nC, nG, nK), dtype=np.float32)
...
def test_beta_upd_fixed_point_safe_regime():
    ...
    beta_naive = beta_upd_naive(beta_init, mu, Ac, eta, theta, gamma, precision, counts)
    beta_fp    = beta_upd_fixed_point(beta_init.copy(), mu, Ac, eta, theta, gamma,
                                precision, counts, class_prob, max_iter=5)

    max_diff = np.abs(beta_naive - beta_fp).max()
...
def test_beta_upd_fixed_point_diverges_in_realistic_regime():
    ...
    """
    beta_init, mu, Ac, eta, theta, gamma, precision, counts, class_prob = (
...
def test_beta_upd_optimized_robust_in_realistic_regime():
    ...
    beta_naive = beta_upd_naive(beta_init, mu, Ac, eta, theta, gamma, precision, counts)
    beta_pcg   = beta_upd_optimized(beta_init.copy(), mu, Ac, eta, theta, gamma,
                              precision, counts, class_prob, max_iter=50)

    max_diff = np.abs(beta_naive - beta_pcg).max()
